...
<%@include file="/templates/admin/inc/sidebar.jsp" %>
      ...
      <section id="main-content">
		  ...
		  <div class="row">
				<div class="col-lg-12">
					<h3 class="page-header"><i class="fa fa-table"></i> Comment Video</h3>
					<ol class="breadcrumb">
						<li><i class="fa fa-home"></i><a href="<%=request.getContextPath()%>/admin/index">Home</a></li>
						<li><i class="fa fa-table"></i><a href="<%=request.getContextPath()%>/admin/commentVideo">Bình luận Video</a></li>
				...
				<div style="margin-left: 16px; margin-top: -14px; margin-bottom: 5px;">
					...
					<form class="navbar-form" style="display: inline-block;" action="<%=request.getContextPath()%>/admin/commentVideo/search" method="get">
	  ...
	  %>
                          ...
                          <table class="table table-striped table-advance table-hover">
                           <tbody>
                              ...
                              <tr>
                                 ...
                                 <td><a href="<%=request.getContextPath()%>/video/detail?vid=<%=objComment.getVideo_id()%>"><%=objComment.getName_video() %></a></td>
                                 <td><%=objComment.getName() %></td>
                                 <td><%=objComment.getEmail() %></td>
                                <td><%=objComment.getWebsite() %></td>
                                 <td><%=objComment.getContent() %></td>
                                 <td id="activeVideo-<%=objComment.getId() %>">
                                 	<%
                                 		if (objComment.getActive()==1){
                                 	%>
                                 	<a onclick="activeVideo(<%=objComment.getId() %>, <%=objComment.getActive() %>)" class="btn btn-success" href="javascript:void(0)" ><i class="icon_check_alt2"></i></a>
                                	<% }else{  %>
                                	<a onclick="activeVideo(<%=objComment.getId() %>, <%=objComment.getActive() %>)" class="btn btn-warning" href="javascript:void(0)" ><i class="icon_upload"></i></a>
                                	<%} %>
                                  </td>
                                      <td>
                                       <a class="btn btn-danger" onclick="return confirm('Bạn có muốn xóa không')"  href="<%=request.getContextPath()%>/admin/commentVideo/del?covid=<%=objComment.getId()%>"><i class="icon_close_alt2"></i></a>
                                  </td>
                                          
                              </tr>
                              
                            <script>
								function  activeVideo(id, active) {
									$.ajax({
										url: '<%=request.getContextPath()%>/admin/commentVideo/active',
										type: 'POST',
										cache: false,
										data: {
												//Dữ liệu gửi đi
          ...
          <div class="pagination" style="margin-left: 23px;   margin-top: -31px;">           
			...
			<%
				...
				String urlSEOFirst = request.getContextPath()+"/admin/commentVideo?page=1";
				String urlSEOLast = request.getContextPath()+"/admin/commentVideo?page="+sumPage;
				String urlSEOPre = request.getContextPath()+"/admin/commentVideo?page="+pre;
				String urlSEONext = request.getContextPath()+"/admin/commentVideo?page="+next;
				%>
				...
				if (sumPage <4) {
					for (int i=1; i<=sumPage;i++){
						...
						String urlSEO = request.getContextPath()+"/admin/commentVideo?page="+i;
						if (i==1){
							%>	
				...
				for (int i=current_page; i<=current_page+3;i++){
					...
					String urlSEO = request.getContextPath()+"/admin/commentVideo?page="+i;if (i==1){
						%>	
						<a <%=active %> style=" text-align: center;" href="<%=urlSEO%>"><%=i %></a>
						<%} else {%>
						<a <%=active %> style=" text-align: center;" href="<%=urlSEO%>">|<%=i %></a>
